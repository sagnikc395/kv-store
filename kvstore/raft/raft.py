...
class ConsensusModule:
    def __init__(
        ...
        self._suppress_election_without_quorum = False
        ...
        while True:
            ...
            with self.mu:
                ...
                if time.monotonic() - self.election_reset_event >= timeout:
                    if (
                        self._suppress_election_without_quorum
                        and not self.server.has_connected_quorum()
                    ):
                        self.dlog("election timer: no connected quorum, waiting")
                        self.election_reset_event = time.monotonic()
                        continue
    ...
    def _become_follower(
        self, term: int, *, suppress_election_without_quorum: bool = False
    ) -> None:
        ...
        self._suppress_election_without_quorum = suppress_election_without_quorum
        self.election_reset_event = time.monotonic()
        threading.Thread(target=self.run_election_timer, daemon=True).start()
                ...
                with self.mu:
                    if (
                        ...
                        self._become_follower(
                            self.current_term, suppress_election_without_quorum=True
                        )
                        return
                    if self.state != CMState.Leader:
                        return
