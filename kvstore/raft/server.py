...
class Server:
    ...
    def has_connected_quorum(self) -> bool:
        """Return True when this server can reach enough peers to win a term."""
        with self.mu:
            connected_peers = sum(1 for peer in self.peer_clients.values() if peer is not None)
        return (connected_peers + 1) * 2 > len(self.peer_ids) + 1
